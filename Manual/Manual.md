# 音色野郎　操作説明　(Version 1.00)
***

## 目次
- [音色野郎　操作説明　(Version 1.00)](#音色野郎操作説明version-100)
  - [目次](#目次)
...
1. [**Z-MUSIC Ver 2.08**]<br>
本ソフトウェアはFM音源ドライバとして**Z-MUSIC**を前提としており、他のドライバでは動作致しません。**（Ver 3には対応していません）**
...
MML書式はZ-MUSIC Ver2.08に準拠しておりVer3.0との互換性はありません。<br>
出力ファイル名は「ZmsData.ZMS」固定です。<br>
また、演奏データをMMLに変換する際に音長の「丸め」が発生しますので、実際の演奏とは音符の長さが異なったMMLデータが出力される事があります。
</td>
</tr>
<tr>
...
<td>
音色データを新規作成します。<br>
新規追加された音色データには、あらかじめサンプルの音色パラメータが設定されています。　作成された音色データは、音色データ群の末尾に追加されます。</td>
</tr>
<tr>
...
<table>
    ...
    <tr><td>[1]～[8]</td><td>音程（オクターブ）切替</td></tr>
    <tr><td>[Q]～[P]</td><td>音色切替</td></tr>
...
<table>
    ...
    <tr><td>[Z]</td><td>「ド」</td></tr>
    <tr><td>[S]</td><td>「ド#」</td></tr>
    <tr><td>[X]</td><td>「レ」</td></tr>
    <tr><td>[D]</td><td>「レ#」</td></tr>
    <tr><td>[C]</td><td>「ミ」</td></tr>
    <tr><td>[V]</td><td>「ファ」</td></tr>
    <tr><td>[G]</td><td>「ファ#」</td></tr>
    <tr><td>[B]</td><td>「ソ」</td></tr>
    <tr><td>[H]</td><td>「ソ#」</td></tr>
    <tr><td>[N]</td><td>「ラ」</td></tr>
    <tr><td>[J]</td><td>「ラ#」</td></tr>
    <tr><td>[M]</td><td>「シ」</td></tr>
</table>
</td>
</tr>
<tr>
...
<td>
キーボードでの演奏を記録します。<br>
その際、<strong>[演奏対象トラック]</strong>で選択されたトラックのデータとして記録されます。
</td>
</tr>
<tr>
...
<table border="1">
    ...
    <tr>
        ...
        <td>現在選択している音色番号を表示します。　表示は「音色番号／音色総数」形式となっており「音色番号」が現在選択し編集中の音色番号、「音色総数」が現在システムに登録されている音色の総数を表します。　「音色番号」欄に値を入力または右側の矢印（▲および▼）をクリックし値を増減させる事で、選択する音色を切り替えられます。</td>
    </tr>
    <tr>
        <td>[ 音色バンク ]欄<br><img src="Image/Man015.png"></td>
        <td>
            登録されている音色をキーボードに割り当てる入力欄です。　数字の「１」～「１０」はキーボードの「Ｑ」～「Ｐ」に対応しており、例えば演奏中に「Ｅ」キー押下で音色バンク欄の「３」に入力されている音色に切り替わり、「Ｐ」キーを押下する事で音色バンク欄の「１０」に入力されている音色に切り替わります。<br>
            <dl>
                <dt>■入力欄とキーの対応</dt>
                <dd>[1] …キーボードの[Q]</dd>
                <dd>[2] …キーボードの[W]</dd>
    ...
    <tr>
        ...
        <td>
        ースロットマスクー<br>
        オペレータのON/OFFを設定します。　左横に<strong>[M1]</strong>～<strong>[C2]</strong>のオペレータ名が表示されており、それぞれ個別にON/OFFを設定できます。　ON状態で「●」が表示され、OFF状態で空白が表示されます。　OFF状態では、当該オペレータへの設定値（<strong>[AR]</strong>～<strong>[AME]</strong>）が無効となります。
        </td>
    </tr>
    <tr>
        ...
        入力値の範囲は0～31です。　数値が小さいとジワッと徐々に出る音になり、逆に数値が大きくなるほど音の立ち上がりが速くなりピアノのようハッキリした音の出方になります。<br>
...
<table border="1">
    ...
    <tr>
        ...
        <td>
        音色データに関するコメントが表示されます。　新規作成時には初期値として「サンプ
        ル音色(Base)」が設定されています。<br>

...
- 2025/11/02 Version 1.23 バグ修正版
- 2025/11/23 Version 1.24 RS-MIDI実験版＜廃版＞
- 2025/11/24 Version 1.25 RS-MIDI実験版＋バグ対応版
 
## Version 1.25について
- Version 1.25はRS-232C（シリアルポート）を経由してMIDIキーボードからの演奏を行うための実験版です。
- PCキーボード／MIDIキーボードの切替機能を実装していないため、PCキーボードからの演奏が行えません。
- 従来通りPCキーボードでの演奏を行う場合は Version 1.23 をご利用ください。

- MIDIキーボードでの演奏に関しては、作者自身がMIDIキーボードを所有していない為に動作試験が行えておりません。
- そのため、Version 1.25はテスト用バージョンですので仮のリリースとなり、テスト結果次第では削除となる可能性が大ですのでご了承下さい。

- <strong>Version 1.24 はキー入力検出処理のバグが存在したため廃版としました。</strong>
...
## インストール（フロッピーからではなく、ハードディスクから使用する場合）
 - フロッピーイメージの場合、ディスク内の「SndEd123」 ディレクトリを丸ごとエミュレータ上のハードディスクにコピーし、続いて「AUTOEXEC.BAT」を編集して環境変数「PATH」に、先ほどコピーしたディレクトリを追加する事で使用可能になります。
...
## ◆ヘルプ機能の操作変更について◆
バージョン1.21より、ヘルプ表示の操作方法を従来の「右クリック」から「CTRLキーを押しながら右クリック」に変更しています。
スピンボタンの操作方法を「右ボタンクリックでも値を下げられる」ように変更したためです。
暫定的な変更ですので、今後のバージョンで従来の操作方法に戻す可能性もあります。

## ◆簡易演奏モードについて◆
バージョン1.21より、[演奏]ボタンをクリックしなくても現在編集中の音色を確認できるように簡易演奏モードを実装しています。
